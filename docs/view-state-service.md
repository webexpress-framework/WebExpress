...
A resource is a named, centrally loaded query. It names the ViewState service that loads it, the state key its result is reduced into, and the parameters that flow between the state and the query. On mount the ViewState loads every automatic resource, and it reduces the outcome into `state[target] = { items, total, loading, error }`. A control subscribes to that slice and renders it; a stale response that a newer query supersedes arrives as an abort and is ignored. A control re-queries a resource by dispatching the ViewState intent `viewstate/query`, whose reducer merges a patch (for example a new search term and a reset page) and whose effect re-queries the named resource, so one control changes the shared state and every subscribing control re-renders from the result.
...
Bindings become state-oriented. The current binds, which are search, paging, filter, show, hide, disable and darkmode, keep their markup. Their default implementations are reframed so that a source event dispatches an intent that updates a store, rather than calling a method on the bound control directly. Two additional binds complete the picture. A state bind subscribes an element or a control to a store path and reflects it as text, as a value, as visibility or as a class, which is the read direction of a controlled component. A model bind provides two-way binding for inputs, where an input event updates a store path and a store change updates the input, which is the controlled input pattern expressed declaratively. The store the model bind writes is the enclosing ViewState when the bound element also binds a resource, so a writing surface — a quickfilter, a search or WQL prompt, a form or a comment composer — writes into the shared state rather than a private store; this is the write counterpart to a control that renders a resource slice. When the element additionally carries data-wx-model-query, the write routes through the viewstate/query intent, which merges the patch and re-queries the named resource, so a single search or filter surface re-queries the shared data that every bound control re-renders from. This is why the BindSearch and BindFilter control-to-control wires are only needed for a standalone surface: inside a ViewState the surface writes the state the resource reads.
...
|data-wx-model          |Control authors  |model bind       |Two way binding between an input and a store path; the store is the enclosing ViewState when the element also binds a resource, so a writing surface writes into the shared state.
...
|ViewStateRegistry |ViewState id |The ViewState, indexed by its ViewState id and by the resources it declares, resolved by resource binding, by id or by ancestry.
